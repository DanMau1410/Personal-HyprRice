...
# Try to focus Spotify and send Ctrl+Shift+R to reload UI
#if command -v hyprctl >/dev/null 2>&1; then
#  hyprctl dispatch focuswindow 'class:^spotify' >/dev/null 2>&1 && wtype -M ctrl -M shift r -m shift -m ctrl
#fi
echo ""
...
swww img "$THEME_DIR/wallpaper.jpg" --transition-type center --transition-fps 144 --transition-step 255 > /dev/null 2>&1
echo ""
...
case "$THEME" in
    everforest-dark|gruvbox-dark|catppuccin|tokyo-night|kanagawa|nord-darker|noir|nightfox|rose-pine)
...
cp "$THEME_DIR/nvim/lua/chadrc.lua" "$HOME/.config/nvim/lua"

NVIM_SOCKET="/tmp/nvchad_socket"
if [ -S "$NVIM_SOCKET" ]; then
    echo -e "${CYAN}-> Reloading NvChad theme in running Neovim instance...${NC}"
    nvim --server "$NVIM_SOCKET" --remote-expr 'v:lua.require("nvchad.utils").reload()'
else
    echo -e "${YELLOW}-> Neovim not running with --listen. Starting headless reload instead.${NC}"
    nvim --headless -c 'lua require("nvchad.utils").reload()' +qa
fi
echo ""
...
# Hyprlock wallpaper
echo -e "${CYAN}-> Symlinking Hyprlock wallpaper...${NC}"
ln -sf $THEME_DIR/wallpaper.jpg ~/.config/hypr/hyprlock/wallpaper > /dev/null 2>&1
echo ""
